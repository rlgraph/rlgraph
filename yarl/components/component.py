...
class Component(Specifiable):
    ...
    def call_api(self, method, method_owner, *params):
        ...
        #if (scoped_name in self.generated_from_graph_fn or scoped_name in self.defined_externally) \
        #        and name in self.api_methods:
        #    # We need to pass the owner to methods that were generated by passing self as an argument to
        #    # the function defining the method, and then calling self.
        #    #out_op_recs = method(method_owner, *in_op_column.op_records)
        out_op_recs = method(*in_op_column.op_records)
        #else:
        #    assert False  # TODO: TEST
        #    # Here the method has already been bound.
        #    out_op_recs = method(*in_op_column.op_records)
